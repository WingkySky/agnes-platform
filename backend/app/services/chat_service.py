...
- **严格区分"讨论"与"执行"**（这是最重要的规则）：
  ...
  - 只有当用户**明确**使用"生成"、"画"、"创建"、"做一张"等动作词时，才调用 generate_image 工具
...
- **URL 链接处理**：用户可能提供图片 URL 链接作为参考图。如果用户明确要求基于该链接图片生成新图，将 mode 设为 image2image；如果只是讨论链接内容，不要调用工具
- 回复使用中文
"""
...
5. 只输出标题本身，不要输出任何其他文字、解释或说明"""
    ...
    async def _execute_generate_image(
        ...
        if use_ref is False or llm_mode == "text2image":
            ...
            if not use_image:
                logger.info("[Chat] generate_image: 模型要求 image2image，但用户未提供参考图，降级为 text2image")
                final_mode = "text2image"
        else:
        ...
        try:
            ...
            if use_image and attachments and len(attachments) > 0:
                ref = attachments[0]
                # 区分参考图来源：base64 上传 or URL 链接
                if ref.get("image_url"):
                    # URL 链接图片：直接传 image_url（Agnes API 原生支持公网 URL）
